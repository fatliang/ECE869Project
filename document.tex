...
\begin{eqnarray}
&&Pr\{SIR \geq \theta \mid \phi_i',h(\bm{c}_{i,j}') \}\nonumber\\
&=& Pr \{ H \geq \theta r^{\alpha} {\sum_i\sum_j \frac{P_i}{P_1} | \bm{c}_{i,j}' |^{-\alpha} h(\bm{c}_{i,j}')} \mid \phi_i',h(\bm{c}_{i,j}') \} \nonumber\\
&=& e^{-\theta r^{\alpha} {\sum_i\sum_j \frac{P_i}{P_1} | \bm{c}_{i,j}' |^{-\alpha} h(\bm{c}_{i,j}')}}
\end{eqnarray}
So the CCDF of SIR can be denoted by
\begin{eqnarray}
Pr\{SIR \geq \theta\}&=&E\{ e^{-\theta r^{\alpha} {\sum_i\sum_j \frac{P_i}{P_1} | \bm{C}_{i,j}' |^{-\alpha} H(\bm{C}_{i,j}')}} \} \nonumber\\
...
\begin{eqnarray}
&&\mathcal{L}_{I_i} (t) \nonumber\\
&=&\lim_{A \rightarrow \mathbb{R}^2} \sum_k e^{-\Lambda_i (B)} \frac{(\Lambda_i (B))^k}{k!} (\int_{B}\int_{0}^{\infty} P_i |\bm{c}|^{-\alpha} h dhd\bm{c})^k
\end{eqnarray}
